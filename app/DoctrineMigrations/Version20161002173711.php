 ...
 */
class Version20161002173711 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        if ($schema->hasTable('location')) {
            ...
            if (!$schemaTable->hasColumn('vote_setting_id')) {
                $this->addSql('
                    ALTER TABLE `location` ADD `vote_setting_id` INT DEFAULT NULL
                ');                
            }
            
            if (!$schemaTable->hasForeignKey('FK_5E9E89CB579C78A')) {
                $this->addSql('
                    ALTER TABLE `location` 
                    ADD CONSTRAINT `FK_5E9E89CB579C78A` 
                    FOREIGN KEY (`vote_setting_id`) REFERENCES `vote_settings` (`id`)
                ');
            }
            
            if (!$schemaTable->hasIndex('IDX_5E9E89CB579C78A')) {
                $this->addSql('
                    CREATE INDEX `IDX_5E9E89CB579C78A` 
                    ON `location` (`vote_setting_id`)
                ');                
            }
        }
        ...
        if ($schema->hasTable('project')) {
            ...
            if (!$schemaTable->hasForeignKey('FK_2FB3D0EE579C78A')) {
                $this->addSql('
                    ...
                    FOREIGN KEY (`vote_setting_id`) 
                    REFERENCES `vote_settings` (`id`)
                ');        
            }
            
            if (!$schemaTable->hasIndex('IDX_2FB3D0EE579C78A')) {
                $this->addSql('
                    CREATE INDEX `IDX_2FB3D0EE579C78A` 
                    ON `project` (`vote_setting_id`)
                ');
            }
        }
    }
    ...
    public function down(Schema $schema)
    {
        if ($schema->hasTable('location')) {
            ...
            if ($schemaTable->hasForeignKey('FK_5E9E89CB579C78A')) {
                $this->addSql('
                    ALTER TABLE `location` DROP FOREIGN KEY `FK_5E9E89CB579C78A`');   
            }
        }
        ...
        if ($schema->hasTable('location')) {
            ...
            if ($schemaTable->hasForeignKey('FK_2FB3D0EE579C78A')) {
                $this->addSql('
                    ALTER TABLE `project` 
                    DROP FOREIGN KEY `FK_2FB3D0EE579C78A`
                ');
            }
        }
        ...
        if ($schema->hasTable('location')) {
            $schemaTable = $schema->getTable('location');
            if ($schemaTable->hasIndex('IDX_5E9E89CB579C78A')) {
                $this->addSql('DROP INDEX `IDX_5E9E89CB579C78A` ON `location`');                
            }
            if ($schemaTable->hasColumn('vote_setting_id')) {
                $this->addSql('ALTER TABLE `location` DROP `vote_setting_id`');
            }            
        }
